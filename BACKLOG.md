...
### 📋 작업 목록 (Task List)
- [ ] **[AUTH-812]** **다중 계정(부캐) 멀티 프로필 전환 및 합방 게스트 모드 지원 (v0.6.0 예정)**:
  - 1) **멀티 프로필 분리**: 복수 계정(본캐/부캐)의 자격 증명 및 브라우저 프로필 분리 관리
  - 2) **합방 게스트 모드**: 타 스트리머와의 임시 합방 시 기존 계정 정보를 덮어쓰지 않고 독립 세션으로 접속 후 즉시 폐기 가능한 게스트 로그인 모드 제공
- [ ] **[ERR-807]** **전체 도메인 체계적 오류 코드 관리 시스템 (`ERR-[DOMAIN]-[NUM]`) 구축**:
  - 1) 네트워크, 인증, OBS 연동, 스튜디오 API, 파일 I/O 도메인별 표준화된 에러 규격 설계
  - 2) 사용자용 직관적 문구와 개발자용 정밀 시스템 로그(원문, PID, 소켓 상태) 분리 파이프라인 정립
...
- [ ] **[UI-811]** **OBS 네이티브 확인창 & 독 내부 모달 UI/UX 디자인 통일화 (v0.6.0 예정)**:
  - 1) **OBS 브라우저 독 네이티브 다이얼로그(`confirm`, `alert`) 규격 정리**: OBS Studio(CEF)에서 표시되는 다이얼로그와 독 내부 커스텀 모달(후원 설정, 가이드 등)의 톤앤매너 및 사용자 경험(UX) 일원화
  - 2) **계정 전환 및 중요 확인 팝업 디자인 개선**: 로그아웃/포트 변경 등 중요 안내 팝업의 디자인 일체화
- [ ] **[CODE-802]** **전체 코드 정밀 리뷰 및 대규모 구조 개선/모듈화 (모든 기능 개발 완료 후 최후 순위 진행)**:
  - 1) 전체 백엔드(Go) 및 프론트엔드(HTML/JS) 누적 코드 정밀 리뷰 진행
  - 2) 단일 5,500줄 HTML 내 JS 로직의 컴포넌트별 모듈 분할 및 관심사 분리
