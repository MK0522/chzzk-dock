...
// UninstallLauncherScriptFromObs: 지정된(또는 자동 감지된) OBS scripts 폴더에서 스크립트 파일을 제거합니다.
func UninstallLauncherScriptFromObs(customDir string) error {
	var targetDir string
	if customDir != "" {
		targetDir = ResolveObsScriptsDir(customDir)
	} else {
	...
	// 2차 시도: 권한 부족 시 Win32 ShellExecuteW "runas"로 관리자 권한 요청
	exePath, err := os.Executable()
	if err != nil {
		return fmt.Errorf("실행 파일 경로 확인 실패: %w", err)
	}
	...
	opPtr, _ := syscall.UTF16PtrFromString("runas")
	filePtr, _ := syscall.UTF16PtrFromString(exePath)
	paramPtr, _ := syscall.UTF16PtrFromString(fmt.Sprintf(`--uninstall-script "%s"`, targetDir))
	...
	return fmt.Errorf("관리자 권한 프로세스 실행 후 파일 삭제를 확인하지 못했습니다")
}
